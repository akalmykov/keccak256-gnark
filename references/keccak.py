...
if __name__ == "__main__":
    ...
    for i in range(len(hexdigest)//hex_chars):
        start_i = hex_chars*i
        print(f"{i}: {hexdigest[start_i:start_i+(hex_chars)]}")
    print("--- decimals (big endian):")
    digest = k.digest()
    for i in range(len(digest)//group_by_bytes):
        start_i = group_by_bytes*i
        print(f"{i}: {int.from_bytes(digest[start_i:start_i+(group_by_bytes)], byteorder='big')}")
    print("--- decimals (little endian):")
    for i in range(len(digest)//group_by_bytes):
        start_i = group_by_bytes*i
        print(f"{i}: {int.from_bytes(digest[start_i:start_i+(group_by_bytes)], byteorder='little')}")
